...
import example.exceptions.CurrencyNotFoundException;
import example.exceptions.InternalErrorException;
import example.model.ExchangeRatesResponse;
import example.rates.responses.RateResponse;
import example.rates.services.ExchangeRatesService;
...
public class RatesEndPoint {
	...
	public void getRates(@Suspended final AsyncResponse async, @PathParam("baseCurrency") final String baseCurrency,
		...
		exchangeRatesService.getExchangeRates(baseCurrency).subscribe(new SingleObserver<ExchangeRatesResponse>() {
